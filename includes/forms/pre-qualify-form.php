...
function cardojo_pre_qualify_application( $shortcode_page ) { ?>
	...
	// Contact Car Dealer
	if( 'POST' == $_SERVER['REQUEST_METHOD'] AND ($_POST['cardojo_form_name'] == "pre_qualify" ) ) {

		if( isset( $_POST['PreQualifyApplicationForm_nonce'] ) && wp_verify_nonce( $_POST['PreQualifyApplicationForm_nonce'], 'PreQualifyApplicationForm_html' ) ){
			...
			<table width="100%" border="0">
				<tr>
					<td colspan="2" style="font-size: 20px;font-weight:bold;color: #333;padding: 10px 0;">'.__('Pre-Qualify Application', 'cardojo').'</td>
			  	</tr>
			  	<tr>
			...
			$website_type = get_option("cardojo_webiste_type");
		  	...
		  	$subject = __('Pre-Qualify Application Form from ', 'cardojo').$site_name;
	...
	<div id="cd-form-wrapp" class="cd-form-wrapp">
	    ...
	    <h4 class="heading"><?php esc_html_e('Pre-Qualify', 'cardojo' ); ?></h4>
		...
		<h5 class="heading">
			...
			<?php esc_html_e('Your Pre-Qualify Request has been sent.', 'cardojo' ); ?><br>
	    ...
	    <form id="submit-lead-cardojo-form" class="contact-form pre-qualify-form-application" action="<?php echo get_permalink(); ?>?form=pre_qualify#cardojo-contact-forms-holder" method="post">
			...
			<?php } ?>
			<input type="hidden" name="cardojo_form_name" value="pre_qualify" />
			...
			<input type="hidden" name="action" value="PreQualifyApplicationForm" />
			<?php wp_nonce_field( 'PreQualifyApplicationForm_html', 'PreQualifyApplicationForm_nonce' ); ?>

			<a id="cardojo_qualify_me" href="#" class="btn btn-default"><i class="fa fa-spinner fa-spin fa-3x fa-fw"></i> <?php esc_html_e( 'Qualify Me', 'cardojo' ) ?></a>
