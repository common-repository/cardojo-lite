...
function cardojo_test_drive_application( $shortcode_page ) { ?>
	...
	// Contact Car Dealer
	if( 'POST' == $_SERVER['REQUEST_METHOD'] AND ($_POST['cardojo_form_name'] == "test_drive" ) ) {

		if( isset( $_POST['TestDriveRequestForm_nonce'] ) && wp_verify_nonce( $_POST['TestDriveRequestForm_nonce'], 'TestDriveRequestForm_html' ) ){
			...
			$textMessage = wp_kses($_POST['lead_comments']);
			...
			<table width="100%" border="0">
				<tr>
					<td colspan="2" style="font-size: 20px;font-weight:bold;color: #333;padding: 10px 0;">'.__('Test Drive Request', 'cardojo').'</td>
			  	</tr>
			  	<tr>
			  	...
			  	<tr>
					<td>'.__('Driver License State', 'cardojo').'</td>
					<td>'.$lead_driver_license_state.'</td>
			  	</tr>
			  	<tr>
					<td>'.__('Drivers License Number', 'cardojo').'</td>
					<td>'.$lead_driver_license_number.'</td>
			  	</tr>
			  	<tr>
					<td>'.__('Drivers License Exp', 'cardojo').'</td>
			...
			$from = sanitize_email( $_POST['lead_email'] );
			...
			$website_type = get_option("cardojo_webiste_type");
		  	...
		  	$subject = __('Test Drive Request Form from ', 'cardojo').$site_name;
	...
	<div id="cd-form-wrapp" class="cd-form-wrapp">
	    ...
	    <h4 class="heading"><?php esc_html_e('Test Drive', 'cardojo' ); ?></h4>
		...
		<h5 class="heading">
			...
			<?php esc_html_e('Your Test Drive Request has been sent.', 'cardojo' ); ?><br>
	    ...
	    <form id="submit-lead-cardojo-form" class="contact-form test-drive-form-application" action="<?php echo get_permalink(); ?>?form=test_drive#cardojo-contact-forms-holder" method="post">
	    		...
	    		<fieldset>
			      	...
			      	<div class="cardojo-row">
						...
						<div class="cardojo-col-4">
							...
							<label for="lead_driver_license_state" class="control-label"><?php esc_html_e('Drivers License State:', 'cardojo' ); ?></label>
							<input type="text" id="lead_driver_license_state" name="lead_driver_license_state" value="" placeholder="" />

						</div>

						<div class="cardojo-col-4">

							<label for="lead_driver_license_number" class="control-label"><?php esc_html_e('Drivers License Number:', 'cardojo' ); ?></label>
							<input type="text" id="lead_driver_license_number" name="lead_driver_license_number" value="" placeholder="" />

						</div>

						<div class="cardojo-col-4">

							<label for="lead_driver_license_exp" class="control-label"><?php esc_html_e('Drivers License Exp:', 'cardojo' ); ?></label>

							<div class="cardojo-input-has-icon">
								<input class="date_picker_future cardojo-input-icon-left" type="text" id="lead_driver_license_exp" name="lead_driver_license_exp" placeholder="<?php esc_html_e('Select Date', 'cardojo' ); ?>" value="" />
								<span class="fa fa-calendar cardojo-input-icon left" aria-hidden="true"></span>
							</div>
			...
			<?php } ?>
			<input type="hidden" name="cardojo_form_name" value="test_drive" />
			...
			<input type="hidden" name="action" value="TestDriveRequestForm" />
			<?php wp_nonce_field( 'TestDriveRequestForm_html', 'TestDriveRequestForm_nonce' ); ?>

			<a id="cardojo_request_test_drive" href="#" class="btn btn-default"><i class="fa fa-spinner fa-spin fa-3x fa-fw"></i> <?php esc_html_e( 'Request Test Drive', 'cardojo' ) ?></a>
